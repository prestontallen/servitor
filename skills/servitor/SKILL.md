...
3. Before presenting: load the `servitor-review` skill and run it on the
   diff (it reviews the diff against the ticket, so after code exists, not
   on the plan). It writes each criterion pass or fail with evidence, one
